    ...
    <script type="text/javascript">
		...
		function formatMgr(value, row) {
	    	...
	    	if(status == 0){
	    		...
	    		"<a href='javascript:void(0)' onclick='del("+row.id+",1)'>删除</a>";
	    	}else if(status == 2){
		...
		//删除
		function del(id){
			$.messager.confirm('Confirm','确定删除此素材吗?',function(r){
				if (r){
					$.post('${pageContext.request.contextPath}/source/delete',{id:id},function(result){
						if (result.success){
		...
		//完成审核
		function complete() {
			window.location.href="${pageContext.request.contextPath}/courseAttr/tolist";
		}
...
<body class="easyui-layout">
    ...
    <table id="dg" class="easyui-datagrid" title="素材管理列表" class="easyui-datagrid" style="width:100%;height:100%"
			url="${pageContext.request.contextPath}/course/courseSourceList?id=${id}"
	...
	<div id="toolbar" style="padding:2px 5px;">
		<label>课件"${name}"包含以下素材：</label>    
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="complete()">完成审核</a>
